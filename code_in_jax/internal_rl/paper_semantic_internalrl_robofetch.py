...
# Meta нь дээд тал нь MACRO_STEP алхам тутамд шийдвэр гаргахыг албадах хязгаар
MACRO_STEP           = 10     
...
# Оптимизатор
LR_BASE_SFT          = 3e-4   # Worker сургах хурд
LR_META_RL           = 1e-4   # Meta сургах хурд
MAX_GRAD_NORM        = 1.0
...
U_DIM                = 128    # Meta controller кодны хэмжээ
GRU_H_DIM            = 256    # Meta түүх санах ойн хэмжээ
LRANK                = 32     # Low-Rank decoder-ийн хэмжээ

# Switching тохиргоо
SWITCH_TAU           = 0.50   # Үнэлгээний үед switch хийх босго

# Beta integration scaling (хугацааны нэгтгэл)
BETA_MIN             = 0.05
BETA_MAX             = 0.995

# Тогтворжуулалт болон шагнал (Regularization)
SWITCH_ENTROPY_COEFF = 0.01   # Switch шийдвэрийг хэт туйлшрахаас сэргийлэх
U_ENTROPY_COEFF      = 0.001  # Proposal үйлдлийн хайгуулыг дэмжих

TARGET_SW_RATE       = 0.10   # Switch хийх давтамжийн зорилтот түвшин (10 алхамд 1 удаа)
SW_RATE_COEFF        = 1.0

BETA_MEAN_TARGET     = 0.85   # Intent хадгалах дундаж хугацааг уртасгах зорилго
BETA_MEAN_COEFF      = 0.2
BETA_VAR_COEFF       = 0.1
...
class ProposalHead(nn.Module):
    ...
    def __call__(self, h):
        ...
        # Variance багатай эхлүүлэхийн тулд constant(-1.0) ашиглав
...
class BetaConfigurator(nn.Module):
    ...
    def __call__(self, h):
        ...
        # 0.05-аас 0.995 хооронд хязгаарлана
...
class ControllerDecoderHyperNet(nn.Module):
    ...
    Зорилго нь параметрийн тоог хэмнэх болон сургалтыг тогтворжуулах.
    """
    embed_dim: int = EMBED_DIM
    rank     : int = LRANK
    u_dim    : int = U_DIM
    ...
    def __call__(self, u_int):
        ...
        # Tanh scaling нь утгуудыг хэт ихсэхээс сэргийлнэ
        A = jnp.tanh(A) * 0.5
        B = jnp.tanh(B) * 0.5
        b = jnp.tanh(b) * 0.5
        return A, B, b
...
class ResidualIntervention(nn.Module):
    """
    Үүсгэсэн A, B, b матрицуудыг ашиглан Worker-ийн ssum вектор дээр хувиргалт хийж, delta (Δ) гаргаж авна.
        Delta = (ssum @ A) @ B + b
    """
    embed_dim: int = EMBED_DIM
    rank     : int = LRANK
    ...
    def __call__(self, u_int, ssum):
        ...
        # ssum @ A -> Low rank projection
        y = jnp.matmul(ssum[:, None, :], A).squeeze(1)

        # y @ B + b -> Projection back to embedding space
...
class MetaController(nn.Module):
    """
    MetaController-ийн бүх хэсгийг нэгтгэсэн модуль.
    ...
    def __call__(self, h_prev, obs, ssum, u_int_prev, step_in_macro, force_macro_switch=False, rng=None):
        ...
        # Macro boundary (MACRO_STEP) хүрсэн бол switch хийхийг албадна
...
def collect_group_rollouts(env, trainer, group_size=GROUP_SIZE):
    ...
    Дараалал:
        1. Meta         : Орчин -> Intent (u_prop) & Switch (sw)
        2. Integration  : u_int үүсгэх
        3. Intervention : u_int -> delta
        4. Worker       : obs + delta -> Action Tokens
        5. Step         : Environment step
    """
    trajs  = []
    scores = []
    ...
    for _ in range(group_size):
        ...
        while (not done) and (t < MAX_EPISODE_STEPS):
            ...
            # Meta Logprob (Training-д хэрэгтэй)
            ...
            for a in range(ACTION_DIM):
                ...
                act.append(tok)
                lpw = lpw + float(logp[0, a, tok])
            act = jnp.stack(act, axis=0).astype(jnp.int32)

            # Нийт Logprob = Meta + Worker
            logp_worker = float(lpw)
            oldlp_total = float(logp_meta[0]) + logp_worker
            ...
            oldlp_list  .append(float(oldlp_total))
...
#  SCANS FOR GRADIENT FLOW & RECONSTRUCTION

@jax.jit
def meta_forward_scan_actions(params_meta, obs_bt, ssum_bt, step_bt, uprop_bt, sw_bt):
    """
    Сургалтын үед Rollout дээр хийгдсэн үйлдлүүдийг (uprop, sw) ашиглан u_int дарааллыг дахин сэргээх (Reconstruction).
    PPO update хийх үед сургалтын болон цуглуулсан өгөгдлийн төлөвийг зөрүүлэхгүй байх чухал ач холбогдолтой.
    """
    meta = MetaController()
    B, T, _ = obs_bt.shape
    ...
    def scan_fn(carry, x):
        ...
        # Rollout дээрх switch-ийг ашиглан state reconstruct хийх
...
def worker_logprob_scan(params_base, params_interv, obs_bt, step_bt, u_int_bt, actions_bt):
    """
    Worker моделийн logprob-ийг тооцоолохдоо intervention (delta)-гаас хамааралтай болгож, градиент урсах замыг нээх функц.
    """
    base   = BaseARPolicy()
    ...
    def scan_fn(carry, x):
        ...
        # Intervention Delta тооцоолол (Градиент эндээс эхлэн Meta руу урсана)
...
def ppo_update_step(meta_state, ref_params_bundle, base_params, obs, ssum, step, uprop, sw, act, oldlp, adv_ep, decmask, mask, kl_beta):
    """
    MetaController болон Decoder-ийг сургах PPO Loss функц. Worker-ийн параметрүүд өөрчлөгдөхгүй.
    """
    def loss_fn(p_bundle):
        p_meta   = p_bundle["meta"  ]
        p_interv = p_bundle["interv"]

        # Meta төлөвийг сэргээх
        ...
        # Worker Logprob (Delta-гаар дамжсан градиент)
        logp_worker_bt = worker_logprob_scan(
            base_params, p_interv, obs, step, u_int_bt, act
        )

        newlp_total = newlp_meta + logp_worker_bt
        ratio       = jnp.exp(newlp_total - oldlp)
        ...
        beta_pen    = (BETA_MEAN_COEFF * (beta_mean - BETA_MEAN_TARGET)**2) + \
                      (BETA_VAR_COEFF  * (1.0 / (beta_var + 1e-4)))
        ...
        loss = -surr_mean + (kl_beta * kl_mean) + (SW_RATE_COEFF * sw_rate_pen) + beta_pen - ent_bonus

        aux = (kl_mean, ent_u, ent_sw, sw_mean, beta_mean, beta_var)
        return loss, aux
...
def ppo_epoch(trainer, ref_params_bundle, trajs, advs, kl_beta):
    ...
    stats = [0.0] * 7 # loss, kl, ent_u, ent_sw, sw_mean, beta_mean, beta_var
    ...
    for i in range(0, len(trajs), MINI_BATCH_SIZE):
        ...
        stats[0] = float(loss)
        for k in range(6): stats[k+1] = float(aux[k])
...
def main():
    ...
    print(f"  U_DIM: {U_DIM} | GRU_H: {GRU_H_DIM} | Embed: {EMBED_DIM}")
    print("=" * 72 + "\n")
    ...
    for upd in range(1, UPDATES + 1):
        ...
        loss_v, kl_v, ent_u, ent_sw, sw_m, beta_m, beta_v = stats
