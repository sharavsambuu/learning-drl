...
def backpropagate_actor(optimizer_state, actor_model_params, actor_ref_params, critic_model_params, props):
    ...
    def loss_fn(params):
        ...
        # Critic baseline (stop-grad)
        values            = jax.lax.stop_gradient(
            critic_module.apply({'params': critic_model_params}, props[0]).squeeze(-1)
        )

        # GRPO-style group centering (DeepSeek-style): center by mean(R_i), no std division
        ...
        # Reference KL (pi_new || pi_ref), ref is frozen
...
def rollout_trajectory(group_member_id, actor_old_params, critic_model_params, seed):
    ...
    for _ in range(max_steps):
        ...
        # NOTE: PPO-style GRPO needs log pi_old(a|s) stored from the behavior policy
...
try:
    ...
    for episode in range(num_episodes):
        ...
        if flat_states.shape[0] > 0:
            ...
            # PPO-style multiple epochs over the same batch
