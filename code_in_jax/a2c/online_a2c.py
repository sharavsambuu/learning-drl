...
def backpropagate_critic(optimizer_state, critic_model_params, props): # pass optimizer_state and model_params
    ...
    # props[2] - reward
    # props[3] - done
    next_value = jax.lax.stop_gradient(critic_module.apply({'params': critic_model_params}, jnp.asarray([props[1]]))[0][0]) # use module.apply and pass params
    def loss_fn(params):
        value      = critic_module.apply({'params': params}, jnp.asarray([props[0]]))[0][0] # use module.apply and pass params
...
def backpropagate_actor(optimizer_state, actor_model_params, critic_model_params, props): # pass optimizer_state, actor_model_params, critic_model_params
    ...
    # props[3] - done
    # props[4] - action
    value      = jax.lax.stop_gradient(critic_module.apply({'params': critic_model_params}, jnp.asarray([props[0]]))[0][0]) # use module.apply and pass critic_model_params
    next_value = jax.lax.stop_gradient(critic_module.apply({'params': critic_model_params}, jnp.asarray([props[1]]))[0][0]) # use module.apply and pass critic_model_params
    advantage  = props[2]+(gamma*next_value)*(1-props[3]) - value
    def loss_fn(params, advantage_val): # rename advantage to advantage_val to avoid shadowing
        action_probabilities = actor_module.apply({'params': params}, jnp.asarray([props[0]]))[0] # use module.apply and pass params
        probability          = action_probabilities[props[4]]
        log_probability      = -jnp.log(probability)
        return log_probability*advantage_val # use advantage_val here
