...
@jax.jit
def critic_apply(critic_params, state, action):
    return critic_1.apply({"params": critic_params}, state, action)

@jax.jit
def calculate_td_error(critic_1_params, critic_2_params, actor_target_params,
                       state, action, reward, next_state, done):
    noise = jnp.clip(jnp.array(np.random.normal(loc=0, scale=policy_noise, size=action.shape), dtype=jnp.float32), -noise_clip, noise_clip)
    next_action = jnp.clip(actor_target.apply({"params": actor_target_params}, next_state) + noise, -max_action, max_action)
...
@jax.jit
def update_critic(critic_1_params, critic_1_opt_state, critic_2_params, critic_2_opt_state,
                  critic_1_target_params, critic_2_target_params, actor_target_params,
    ...
    def critic_loss_fn(critic_params, state, action, target_q, weights):
        current_q = critic_1.apply({"params": critic_params}, state, action)
        loss = jnp.mean(weights * (current_q - target_q)**2)
        return loss

    grad_fn = jax.value_and_grad(critic_loss_fn)
    critic_1_loss, critic_1_grads = grad_fn(critic_1_params, state, action, target_q, is_weights)
    critic_1_updates, critic_1_opt_state = critic_1_optimizer.update(critic_1_grads, critic_1_opt_state, critic_1_params)
    critic_1_params = optax.apply_updates(critic_1_params, critic_1_updates)

    critic_2_loss, critic_2_grads = grad_fn(critic_2_params, state, action, target_q, is_weights)
    critic_2_updates, critic_2_opt_state = critic_2_optimizer.update(critic_2_grads, critic_2_opt_state, critic_2_params)
    critic_2_params = optax.apply_updates(critic_2_params, critic_2_updates)

    current_q1 = critic_1.apply({"params": critic_1_params}, state, action)
    td_errors = jnp.abs(target_q - current_q1)

    return critic_1_params, critic_1_opt_state, critic_2_params, critic_2_opt_state, critic_1_loss, critic_2_loss, td_errors

@jax.jit
def update_actor(actor_params, actor_opt_state, critic_1_params, state):
    def actor_loss_fn(actor_params, state):
        actor_loss = -jnp.mean(critic_apply(critic_1_params, state, actor.apply({"params": actor_params}, state)))
        return actor_loss

    actor_loss, actor_grads = jax.value_and_grad(actor_loss_fn)(actor_params, state)
...
def soft_update(target_params, params, tau):
    new_target_params = jax.tree_util.tree_map(
        lambda target_param, param: tau * param + (1 - tau) * target_param,
...
for episode in range(num_episodes):
    ...
    while True:
        ...
        init_td_error = calculate_td_error(critic_1_params, critic_2_params, actor_target_params,
                                          jnp.array([state]), jnp.array([action]), jnp.array([reward]),
                                          jnp.array([next_state]), jnp.array([float(done)]))
        ...
        if per_memory.experience_count > batch_size:
            ...
            critic_1_params, critic_1_opt_state, critic_2_params, critic_2_opt_state, critic_1_loss, critic_2_loss, updated_td_errors = update_critic(
                critic_1_params, critic_1_opt_state, critic_2_params, critic_2_opt_state,
                critic_1_target_params, critic_2_target_params, actor_target_params,
                jnp.array(state_batch, dtype=jnp.float32), jnp.array(action_batch, dtype=jnp.float32), jnp.array(reward_batch, dtype=jnp.float32),
                jnp.array(next_state_batch, dtype=jnp.float32), jnp.array(done_batch, dtype=jnp.float32), jnp.array(is_weights, dtype=jnp.float32)
            )
