...
class DuelingQNetwork(nn.Module):
    ...
    def __call__(self, x):
        ...
        v = nn.Dense(features=64)(x)
        v = nn.relu(v)
        v = nn.Dense(features=1)(v)

        a = nn.Dense(features=64)(x)
        a = nn.relu(a)
        a = nn.Dense(features=self.n_actions)(a)

        a_mean = jnp.mean(a, axis=-1, keepdims=True)
        q = v + (a - a_mean)
        return q
...
def td_error_batch(q_network_params, target_q_network_params, batch):
    ...
    q_values      = dqn_module.apply({'params': q_network_params       }, states     )
    q_next_online = dqn_module.apply({'params': q_network_params       }, next_states)
    q_next_target = dqn_module.apply({'params': target_q_network_params}, next_states)

    action_select = jnp.argmax(q_next_online, axis=1)
    target_q      = jnp.take_along_axis(q_next_target, action_select[:, None], axis=1).squeeze(1)

    q_value       = jnp.take_along_axis(q_values, actions[:, None], axis=1).squeeze(1)

    td_target     = rewards + gamma * target_q * (1.0 - dones)
...
def train_step(q_network_params, target_q_network_params, opt_state, batch, is_weights):
    ...
    def loss_fn(params):
        q_values      = dqn_module.apply({'params': params                 }, states     )
        q_next_online = dqn_module.apply({'params': params                 }, next_states)
        q_next_target = dqn_module.apply({'params': target_q_network_params}, next_states)
