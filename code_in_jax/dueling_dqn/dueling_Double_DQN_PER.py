...
class DuelingQNetwork(nn.Module): # Use flax.linen.Module
    ...
    def __call__(self, x): # Use __call__ instead of apply
        ...
        value_stream = nn.Dense(features=1)(value_stream)
        value = value_stream

        # Advantage stream
        advantage_stream = nn.Dense(features=64)(feature_layer)
        advantage_stream = nn.relu(advantage_stream)
        advantage_stream = nn.Dense(features=self.n_actions)(advantage_stream)
...
dqn_module       = DuelingQNetwork(n_actions=n_actions) # Use DuelingQNetwork
...
def calculate_td_error(q_value_vec, next_q_value_vec, target_q_value_vec, action, reward): # No jax.jit here, define function BEFORE vmap
    ...
    target_q_value     = target_q_value_vec[action_select] # select q value using action_select from online network, but evaluate from target network
...
def td_error_batch(q_network_params, target_q_network_params, batch): # Renamed to td_error_batch to avoid name conflict and clarify batch processing
    ...
    target_q_values         = dqn_module.apply({'params': target_q_network_params}, batch[3]) # use target network to evaluate value
